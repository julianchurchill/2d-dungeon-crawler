...
- 2026-05-14 — **ascend() places player at stairs-up position**: `ascend()` in `PlayerActionHandler` was using `dungeonData.stairsPos` (the stairs-down tile) instead of `dungeonData.stairsUpPos` when positioning the player after ascending, causing the player to materialise at the wrong end of the map. `TownGenerator.generate()` now also returns `stairsUpPos` so ascending back to town does not crash.
