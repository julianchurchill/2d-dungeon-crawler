...
- 2026-04-26 — **Look panel invisible on Classic/Modern tilesets**: the look panel was positioned in world-space coordinates but not compensating for camera zoom (Classic and Modern use zoom=2). This caused the panel to render at 2× its intended screen position, placing it off-screen. `LookPanel._setPosition` now divides by the active camera zoom so the panel always appears in the bottom-right corner regardless of tileset.
