...
- 2026-05-02 — **Drop items from inventory**: pressing `D` while the inventory is open drops the highlighted item onto the player's current floor tile. On touch devices a `[DROP]` button is shown in the inventory panel header. The dropped item becomes a floor item that can be picked up again. `GameEvents.INVENTORY_DROP` added.
