...
- 2026-04-25 — **The Darker Way alcove exposure**: the locked alcove (containing the Eclipse Blade and Key to Beyond) could spawn directly accessible from the dungeon corridor when a BSP corridor entered the room from the same side as the alcove. The fix scans all four room edges before placing the divider; if entries exist on both halves the locked room is skipped for that spawn rather than exposing the alcove. A new `LockedRoomPlacer` utility encapsulates the orientation logic and is covered by dedicated unit scenarios.
