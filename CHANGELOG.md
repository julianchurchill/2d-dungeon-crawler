...
- 2026-05-15 — **applyChampionLoot and applyBossLoot passed Item instance to _placeItem instead of type definition**: both methods now pass `enemy.dropItem._typeDef` so `_placeItem` constructs a fresh `Item` from the correct plain type definition object. Passing the `Item` instance directly caused the placed item's `_typeDef` to be an `Item`, breaking `use()` and `isConsumable()`.
- 2026-05-15 — **Unique room entry message suppressed after save/load when room not yet entered**: `_restoreFloor` always called `setRoomRestored` (which suppresses the entry announcement), even when the player had saved on a floor containing a unique room they had not yet walked into. Added `UniqueRoomEntryTracker.restoreFromSave(room, def, wasEntered)` which calls `setRoomRestored` only when the room was already entered, and `setRoom` otherwise so the first-entry message fires correctly. `_restoreFloor` now passes `wasEntered` derived from `floorState.uniqueRooms.entered`.
- 2026-05-14 — **Security: importSave validates floor state shape**: `importSave` now rejects saves with a negative floor number, a `playerX`/`playerY` outside the map bounds, or a `tiles` array whose length does not match `width * height`. Malformed saves return `false` rather than being stored.
- 2026-05-14 — **Security: prototype pollution hardening**: `AchievementStore.loadAchievementStore` now only accepts keys that match a known achievement ID (whitelist built from `ACHIEVEMENTS`); unknown or crafted keys are silently ignored. `GameLifecycleHandler.applyLoadedSave` now calls `applyPlayerStats` (new export in `SaveGame.js`) which copies only the eight known stat fields instead of using `Object.assign`, preventing crafted saves from injecting arbitrary properties into player state. Existing achievement tests updated to use a real achievement ID (`cockroach_killer`).
...
- 2026-05-13 — **Unique room textures lost after portal return**: using a Home Seeking Scroll and returning through the RECALL_PORTAL repainted the unique room (library, armoury, darker way) with default dungeon tiles. `DungeonSnapshot` now captures the active unique room; `returnFromSnapshot` calls `FloorBuilder.paintUniqueRoomTiles` after rebuilding the floor to restore the correct themed tiles.
...
- 2026-05-13 — **Refactor: fix feature envy (top 3)**: extracted `CHAMPION_TINT`/`CHAMPION_SCALE` into `src/utils/EntityConstants.js` (shared by `GameScene`, `SpriteAnimator`, `FloorBuilder`); added `SpriteAnimator.repositionSprite` so `CombatHandler` no longer calls `sprite.setPosition` directly; added `FloorBuilder.attachEnemySprite`/`attachItemSprite` so `PlayerActionHandler.returnFromSnapshot` no longer creates sprites inline.
...
- 2026-05-14 — **Tests: document intended ascend and equip behaviour**: added Cucumber scenarios to `player-action-handler.feature` confirming that (1) `ascend()` lands the player at the stairs-down tile (`stairsPos`) on the destination floor, not the stairs-up tile, and (2) equipping a non-consumable item from the inventory panel leaves the turn state as `INVENTORY` and does not trigger enemy turns — both intentional by design.
