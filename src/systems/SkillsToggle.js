 ...
 *
 * - `PLAYER_INPUT` → opens the skills panel (transitions to SKILLS, returns true)
 * - `SKILLS`       → closes the skills panel (transitions to PLAYER_INPUT, returns true)
 ...
 */
export function applySkillsToggle(turnManager) {
  ...
  if (turnManager.state === TURN_STATE.PLAYER_INPUT) {
    turnManager.setState(TURN_STATE.SKILLS);
    return true;
  }
  return false;
}
